	...
	*/
	vector<T>(const int n, const T &value){
		...
		vector<T>(const vector<T>& other){
			...
			data_array = new T[current_capacity];
			//data_array = new T[INITIAL_CAPACITY];
			for (size_t i = 0; i < other.current_capacity; i++) {
		...
		vector<T>(T& start, T& end){	
			...
			current_capacity = INITIAL_CAPACITY;
                        data_array = new T[INITIAL_CAPACITY];
			//T* point = &start;
			if (current_capacity < num_items) {
                               reserve(num_items);
                        }
							
			for (size_t j = 0; j < current_capacity; j++) {
				data_array[j] = *point;
				point++;
			} 
    ...
    void vector<T>::concat(const vector<T>& other){
  		//To do: implement the function
		current_capacity = current_capacity + other.current_capacity;
		
		T* temp = new T[current_capacity];
		for (size_t i = 0; i < num_items; i++) {
			temp[i] = data_array[i];
		} 	
		for (size_t i = 0; i < other.num_items; i++) {
			temp[i + num_items] = other.data_array[i];
		}
		num_items = num_items + other.num_items;
		delete [] data_array;
		data_array = temp;
    }
	...
	vector<T>& vector<T>::operator+(const vector<T>& other){
		...
		//*this is the object including data_array that invokes + operator.
		return *this;				
	}
